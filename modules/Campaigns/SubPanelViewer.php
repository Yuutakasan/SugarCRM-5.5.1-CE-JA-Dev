...
elseif(empty($_REQUEST['mkt_id']) && !empty($record)) {
    $latest_marketing_query = "select id, name, date_modified from email_marketing where campaign_id = '{$record}' order by date_modified desc";
    $result =$GLOBALS['db']->query($latest_marketing_query);
