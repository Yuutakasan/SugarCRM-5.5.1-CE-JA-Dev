	...
	if($view == 'month') {
		for($i=0; $i<strlen($dateFormat['date']); $i++) {
			switch($dateFormat['date']{$i}) {
				case "Y":
					$displayStr .= $date_time->year . $mod_strings['LBL_YEAR'];
					break;
				case "m":
					$displayStr .= " " . $date_time->get_month_name();
					break;
			}
		}
		echo $displayStr;
		...
		if($view == 'week' || $view == 'shared') {
			...
			for($i=0; $i<strlen($dateFormat['date']); $i++) {
				switch($dateFormat['date']{$i}) {
					case "Y":
						$displayStr .= $first_day->year . $mod_strings['LBL_YEAR'];
						break;
					case "m":
						$displayStr .= " " . $first_day->get_month_name();
						break;
					case "d":
						$displayStr .= " " . $first_day->get_day() . $mod_strings['LBL_DAY'];
						break;
				}
			}
			...
			for($i=0; $i<strlen($dateFormat['date']); $i++) {
				switch($dateFormat['date']{$i}) {
					case "Y":
						$displayStr .= $last_day->year . $mod_strings['LBL_YEAR'];
						break;
					case "m":
						$displayStr .= " " . $last_day->get_month_name();
						break;
					case "d":
						$displayStr .= " " . $last_day->get_day() . $mod_strings['LBL_DAY'];
						break;
				}
			}
			echo $displayStr;
				...
				for($i=0; $i<strlen($dateFormat['date']); $i++) {
					switch($dateFormat['date']{$i}) {
						...
						case "m":
							...
							break;
						case "d":
							$displayStr .= " " .$date_time->get_day() . $mod_strings['LBL_DAY'];
							break;
					}
				}
				echo $displayStr . " " . $date_time->get_day_of_week();
//jkim
			} else
				if($view == 'year') {
