...
class EmailUI {
	...
	function displayEmailFrame() {
        ...
        $quicksearchAssignedUser .= "sqs_objects['advancedSearchForm_assigned_user_name']={$json->encode($qsd->getQSUser())};";
        $qsd->setFormName('Distribute'); 
        $quicksearchAssignedUser .= "sqs_objects['Distribute_assigned_user_name']={$json->encode($qsd->getQSUser())};";
...
eoq;
    ...
    function generateComposePackageForQuickCreate($composeData,$fullLinkUrl)
    {
    	...
    	$j_quickComposeOptions = $json->encode($quickComposeOptions);
...
eoq;
    ...
    function getRelatedEmail($beanType, $whereArr, $relatedBeanInfoArr = ''){
		...
		if($beanType === 'LBL_DROPDOWN_LIST_ALL')
			$searchBeans = array("users", "contacts", "leads", "prospects", "accounts");
...
eoq;
	...
	function jsonOuput($data, $resultsParam, $count=0, $fromCache=true, $unread=-1) {
		...
		return $json->encode($jsonOut, false);
	}
	/**
