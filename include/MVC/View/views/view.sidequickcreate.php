...
*/
//jkim add metafile経由で参照するように修正
		if(file_exists('custom/'.$file)) {
			$this->ev->setup($this->module, null, 'custom/'.$file);
		} else {
	 		if(file_exists('custom/modules/'.$this->module.'/metadata/metafiles.php')){
				require_once('custom/modules/'.$this->module.'/metadata/metafiles.php');
				if(!empty($metafiles[$this->module]['sidecreateviewdefs'])){
					$file = $metafiles[$this->module]['sidecreateviewdefs'];
				}
			...
			}elseif(!file_exists('modules/' . $this->module . '/metadata/sidecreateviewdefs.php')) {
				return false;
			}
					$GLOBALS['log']->fatal("file = $file");
			$this->ev->setup($this->module, null, $file);
		} 
//jkim end
