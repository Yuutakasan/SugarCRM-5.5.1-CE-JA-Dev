...
if(savePressed||enterPressed){setTimeout(forceSubmit,2100);}else if((typeof(tabPressed)!=='undefined')&&tabPressed){document.getElementById('emailAddressPrimaryFlag'+index).focus();}}
...
if(emailView=='EditView'){theForm.submit();}else if(emailView=='QuickCreate'){SUGAR.subpanelUtils.inlineSave(theForm.id,theForm.module.value.toLowerCase());}}}// End of File include/SugarEmailAddress/SugarEmailAddress.js
...
var toreplace=this.field+"_collection_extra_0";var re=new RegExp(toreplace,'g');parentNode.innerHTML=parentNode.innerHTML.replace(re,this.field+"_collection_extra_"+this.fields_count);}else if(currentNode.tagName&&currentNode.tagName=='SPAN'){currentNode.id=/_input/.test(currentNode.id)?this.field_element_name+'_input_div_'+this.fields_count:this.field_element_name+'_radio_div_'+this.fields_count;input_els=currentNode.getElementsByTagName('input');for(x in input_els){if(input_els[x].tagName&&input_els[x].tagName=='INPUT'){this.process_node(parentNode,input_els[x],values);}}}else if(currentNode.name){var toreplace=this.field+"_collection_0";var re=new RegExp(toreplace,'g');var name=currentNode.name;var new_name=name.replace(re,this.field+"_collection_"+this.fields_count);switch(name){case toreplace:sqs_id=this.form+'_'+new_name;if(typeof this.sqs_clone!='undefined'){var sqs_clone=YAHOO.lang.JSON.stringify(this.sqs_clone);eval('sqs_objects[sqs_id]='+sqs_clone);for(var pop_field in sqs_objects[sqs_id]['populate_list']){if(typeof sqs_objects[sqs_id]['populate_list'][pop_field]=='string'){sqs_objects[sqs_id]['populate_list'][pop_field]=sqs_objects[sqs_id]['populate_list'][pop_field].replace(RegExp('_0','g'),"_"+this.fields_count);}}
...
SE.accounts = {
    ...
    */
	...
	smtp_authenticate_field_display : function() {
		...
		var smtp_auth_div = document.getElementById("smtp_auth_div");
		smtp_auth_div.style.display = smtpauth_req.checked ? "inline" : "none";
	},

    /**
     * Presets default values for Gmail, accepted known connections are: 
     * Protocol: SSL, Port: 465.  Protocol: TLS : 587
     */
...
SE.addressBook = {
    ...
    removeContact : function() {
        ...
        if(confirm(app_strings.LBL_EMAIL_CONFIRM_DELETE)) {
            ...
            for(var i=0; i < rows.length; i++) {
                ...
                if (rowData[1].indexOf('address-contact') > -1) {
                    if(str != '') {
                        str += "::";
                    }
                    str += rowData[0];
                } else if (rowData[1].indexOf('address-exp-contact') > -1) {
                    if(str != '') {
                        str += "::";
                    }
                    str += rowData[0].substring(2);
                }
            }
    ...
    */
    ...
    insertContactToField : function(event,obj) {
        
        //Iff cc or bcc fields are hidden, show them.
        if(obj.address_type =='cc' ||obj.address_type =='bcc' )
            SE.composeLayout.showHiddenAddress(obj.address_type,obj.idx);
        
        var contactsDialogue = SE.addressBook.contactsDialogue;
        var target = document.getElementById(obj.addressElement);
        var contacts = SE.addressBook.grid.getSelectedRows();
        for (var i=0; i < contacts.length; i++) {
            var data = SE.addressBook.grid.getRecord(contacts[i]).getData();
            target.value = SE.addressBook.smartAddEmailAddressToComposeField(target.value, data.name.replace(/&#039;/gi,'\'').replace(/&quot;/gi,'"') + ' <' + data.email.replace(/&#039;/gi,'\'') + '>');
        }
        //Untoggle selected records
        SE.addressBook.grid.toggleSelectAll(false);
        SE.composeLayout.showAddressDetails(target);
        //Inform user addresses have een added
        ajaxStatus.flashStatus(mod_strings.LBL_RECIPIENTS_HAVE_BEEN_ADDED, 2000);
    },
    /**
    ...
    */
    ...
    addDocumentField : function(idx) {
        ...
        documentCup.innerHTML = "<input type='hidden' name='document" + idx + index + "' id='document" + idx + index + "' />" +
                ...
                // document name field
                "<input value='' size='20' disabled='true' type='text' name='documentName" + idx + index + "' id='documentName" + idx + index + "' />" +
                // select button
...
var AjaxObject = {
	...
	composeCache : function(o) {
		...
		if(overlayPanel)
		  overlayPanel.subscribe('hide',function() { YAHOO.util.Dom.setStyle('addressFrom' + idx,'visibility','visible');});
	...
	sendEmailCleanUp : function(o) {
		...
		//history subpanel
		if ( (typeof(action_sugar_grp1) != 'undefined') && (action_sugar_grp1 == 'DetailView') )
...
AjaxObject.detailView = {
	...
	showQuickCreateForm : function(o) {
		...
		if (ret.html) {
			ret.html = ret.html.replace("SugarEmailAddress.js", "");
		}
		...
		if (editForm) {
		  ...
		  prefillEmailAddresses(ret.module + "emailAddressesTable", ret.emailAddress);
		}
	},
		...
		init : function() {
			...
			var colModel = 
				[
					...
					{
						label: app_strings.LBL_EMAIL_DATE_SENT_BY_SENDER,
						width: widths[4], 
						sortable: true,
						resizeable: true,
                        key: 'date'
